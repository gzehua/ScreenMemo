...
<details>
<summary>动态重建后如果有一段时间没有生成动态，如何补齐？</summary>

- 进入“动态”页面，点击右上角“重建动态”按钮打开任务面板
- 面板的原操作区提供两个操作按钮：左侧红色“重建”，右侧墨绿色“补全”
  - “重建”会弹出二次确认，说明会清空当前动态并从最老截图全量生成
  - “补全”会弹出二次确认，说明会扫描每一天并只补齐缺失日期、缺失时间窗或缺失总结
- 任务运行后，两个操作按钮会切换为按真实任务命名的停止按钮，例如“停止重建”或“停止补全”；停止后若任务仍可续跑，会显示“退出补全 / 继续补全”或“退出重建 / 继续重建”，退出只清除本次任务状态，不删除已生成的动态
- 面板标题和状态角标始终以真实任务为准，例如“当前后台任务：动态补全”“补全 · 运行中”；补全任务的进度条使用墨绿色，重建任务保持原进度条颜色
- 通知栏也会跟随真实任务模式显示“正在重建动态”或“正在补全动态”，完成、失败和停止文案同样区分重建 / 补全
- 补全任务复用重建任务的后台队列、通知进度和并发天数设置；不会清空已有动态
- 补全会先整理已有顶层动态的时间交错，再按截图时间扫描缺失窗口；部分重叠的窗口会推进到空档后重建，短缺口会并入相邻动态，不生成碎片动态
- 每条补全结果生成后仍会走“与上一条动态合并”的完整判断链路；若合并成功，会继续向更早的动态递归检查，合并与补全都遵循现有 AI 请求间隔
</details>
